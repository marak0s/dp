...
def evaluate_decoders(config: dict[str, Any], ds: SignalsDataset) -> pd.DataFrame:
    code = config["code"]
    enabled = config["experiment"]["decoders"]
    trellis = build_trellis(code["constraint_length"], tuple(code["polynomials"]))
    n_out = len(code["polynomials"])
    nv = NeuralViterbiDecoder(n_out=n_out)
    nb = NeuralBCJRDecoder(n_out=n_out)
    ...
    for i in range(ds.u.shape[0]):
        ...
        if "neural_viterbi" in enabled:
            t0 = time.perf_counter()
            uh = neural_viterbi_decode(llr, trellis, nv)
            dt = time.perf_counter() - t0
        ...
        if "neural_bcjr" in enabled:
            t0 = time.perf_counter()
            _, uh = neural_bcjr_decode(llr, trellis, nb)
            dt = time.perf_counter() - t0
